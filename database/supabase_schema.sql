-- Web3 University Supabase Schema
-- This schema is compatible with Supabase and includes Row Level Security (RLS)

-- Enable UUID extension (usually available by default in Supabase)
...
CREATE POLICY "Anyone can view published courses" ON courses FOR SELECT USING (status = 'published' AND is_active = true);
CREATE POLICY "Admins and mentors can manage courses" ON courses FOR ALL USING (
    EXISTS (
        SELECT 1 FROM users 
